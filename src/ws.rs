...
impl<B: BufferStrategy> WebSocket<B> {
	...
	pub fn recv(&mut self, read: &mut dyn io::Read) -> io::Result<usize> {
		let buf = self.inner.strategy.recv_rx(&mut self.rxbuffer);
		let n = read.read(buf)?;
		self.inner.strategy.recvn_rx(&mut self.rxbuffer, n);
		Ok(n)
	}
	...
	pub fn dispatch(&mut self, handler: &mut dyn Handler) -> Result<()> {
		...
		let mut rxbuffer = self.inner.strategy.take_rx(&mut self.rxbuffer);
		...
		let result = loop {
			...
			let buffer = self.inner.strategy.get_rx(&mut rxbuffer, offset);
		...
		if offset > 0 {
			let _ = self.inner.strategy.consume_rx(&mut rxbuffer, offset);
		}

		// Carefully place the rxbuffer back in its place
		let _ = self.inner.strategy.store_rx(&mut self.rxbuffer, rxbuffer);
	...
	pub fn send(&mut self, dest: &mut dyn io::Write) -> io::Result<()> {
		if self.inner.strategy.len_tx(&mut self.inner.txbuffer) == 0 {
			return Ok(());
		}
		loop {
			match dest.write(self.inner.strategy.bytes_tx(&self.inner.txbuffer)) {
				Ok(n) => {
					self.inner.strategy.consume_tx(&mut self.inner.txbuffer, n);
					return Ok(());
				},
	...
	pub fn send_all(&mut self, dest: &mut dyn io::Write) -> io::Result<()> {
		let len = self.inner.strategy.len_tx(&mut self.inner.txbuffer);
		if len > 0 {
			...
			dest.write_all(self.inner.strategy.bytes_tx(&self.inner.txbuffer))?;
			self.inner.strategy.consume_tx(&mut self.inner.txbuffer, len);
		}
		Ok(())
	}
	pub fn send_fn<F, E>(&mut self, mut f: F) -> result::Result<(), E> where F: FnMut(&[u8]) -> result::Result<usize, E> {
		let len = self.inner.strategy.len_tx(&mut self.inner.txbuffer);
		if len > 0 {
			let n = f(self.inner.strategy.bytes_tx(&self.inner.txbuffer))?;
			self.inner.strategy.consume_tx(&mut self.inner.txbuffer, n);
		}
		Ok(())
	}
