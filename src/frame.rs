...
pub(crate) fn handle_frame<B: BufferStrategy>(buffer: &mut [u8], ws_rx: FrameSTM, ws: &mut WebSocket<B>, handler: &mut dyn Handler) -> Result<usize> {
	...
	match header.opcode {
		crate::Opcode::CONTINUE => {
			ws.inner.strategy.append_msg(&mut ws.payload, payload);
			if header.fin {
				unimplemented!()
			}
		},
...
///
/// * Returns `Ok(0)` if there were not enough bytes in the stream to fully decode the frame header.
...
///
/// * Returns `Ok(0)` if the input stream does not contain at least `header.payload_len` bytes.
///   Accumulate more data from the source before trying to decode again.
///
/// # Examples
///
